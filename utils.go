...
// Update the .gitignore file by replacing the SYNCAPP SECTION with the sync patterns
func updateGitIgnore(files []string) error {
	...
	sectionStart := "# SYNCAPP SECTION START"
	sectionEnd := "# SYNCAPP SECTION END"

	// Generate the new content for the SYNCAPP section
	...
	// Regular expression to match the existing SYNCAPP section
	sectionRegex := regexp.MustCompile(`(?s)# SYNCAPP SECTION START.*?# SYNCAPP SECTION END`)

	// Replace the SYNCAPP section if it exists, or append it if not
